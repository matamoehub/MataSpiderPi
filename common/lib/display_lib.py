...
class Display:
    ...
    def icon(self, name: str, seconds: float | None = None):
        return self.shape(name, seconds=seconds)

    def emoji(self, name: str, seconds: float | None = None):
        return self.shape(name, seconds=seconds)

    def icons(self) -> list[str]:
        return sorted(name for name in _SHAPES if not name.startswith("test_"))
