...
class TM1640(object):
    ...
    def clear(self):
        """熄灭点阵所有led(turn off all LEDs of the matrix)"""
        self.display_buf = [0] * 16
        self.update_display()

    def set_bit(self, x, y, s):
        """设置(x, y)处的led状态，0或者1，0表示熄灭(Set the LED status at (x, y) to 0 or 1, with 0 indicating off)"""
        self.display_buf[x] = (self.display_buf[x] & (~(0x01 << y))) | (s << y)

    def set_number(self, number):
        """数码管显示数字(digital tube displays numbers)"""
    ...
    def set_buf_horizontal(self, buf):
        """设置显示的二进制列表，横向(set the list of binary to be displayed horizontally)"""
        l = zip(*buf[::-1])
        self.display_buf = [int(''.join(i), 2) for i in l]
    
    def set_buf_vertical(self, buf):
        """设置显示的二进制列表，竖向(set the list of binary to be displayed vertically)"""
        self.display_buf = [int(i, 2) for i in buf]

    def update_display(self):
        """刷新显示(refresh the display)"""
